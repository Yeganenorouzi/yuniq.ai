...
final class ChatController implements HookableInterface {
	...
	public function get_public_config() {
		return new WP_REST_Response(
			array(
				...
				'video_enabled'     => (bool) $this->settings->get( 'video_enabled' ),
				'video_url'         => $this->settings->get( 'video_url' ),
				'video_title'       => $this->settings->get( 'video_title' ),
	 ...
	 * @return WP_REST_Response
	 */
	private function error( $message, $status, $session_id = '' ) {
		$payload = array(
			'success' => false,
			'error'   => $message,
		);
